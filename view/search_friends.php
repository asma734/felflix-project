...
if(isset($_GET['add_friend'])) {
    ...
    try {
        $cnx->prepare("INSERT IGNORE INTO friends(user_id, friend_id, status) VALUES(?, ?, 'accepted')")->execute([$user['id'], $f_id]);
        $cnx->prepare("INSERT IGNORE INTO friends(user_id, friend_id, status) VALUES(?, ?, 'accepted')")->execute([$f_id, $user['id']]); // Mutual for simplicity
    } catch(PDOException $e) {}
      ...
      <?php else: ?>
          <div style="display:grid; gap:15px;">
              <?php foreach($results as $r): 
                  // Check friend status
                  $fchk = $cnx->prepare("SELECT * FROM friends WHERE user_id=? AND friend_id=?");
