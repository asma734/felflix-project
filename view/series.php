<?php
// ============================================================
//  VUE : Mosalsalat m7ar7ra (Premium Series Section)
// ============================================================
session_start();
...
$filters = [
    'q'           => trim($_GET['q'] ?? ''),
    'type'        => 'series', // Fixé pour cette page
    'genre_id'    => (int)($_GET['genre_id']    ?? 0),
    'country_id'  => (int)($_GET['country_id']  ?? 0),
    'language_id' => (int)($_GET['language_id'] ?? 0),
    'year_from'   => (int)($_GET['year_from']   ?? 1980),
    'year_to'     => (int)($_GET['year_to']     ?? 2025),
    'min_rating'  => (float)($_GET['min_rating'] ?? 7.0),
];
...
$pageTitle  = 'Mosalsalat m7ar7ra 🌶';
$activePage = 'series';
require_once '_header.php';
?>

<div class="wrap" style="padding-top:120px; padding-bottom:60px">
    <!-- Header Premium -->
    <div class="search-header-premium" style="margin-bottom: 40px;">
        <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 10px;">
            <div style="font-size: 3rem; color: #e63946; filter: drop-shadow(0 0 10px rgba(230,57,70,0.5));">📺</div>
            <h1 style="font-family: 'Space Grotesk', sans-serif; font-weight: 800; font-size: 3.5rem; margin: 0; color: #fff; letter-spacing: -1px;">Mosalsalat <span style="color: #f5a623;">m7ar7ra</span></h1>
        </div>
        <p style="color: var(--muted); font-size: 1rem; margin-left: 5px;"><?= number_format($total, 0, '.', ' ') ?> séries trouvées dans cette catégorie</p>
    </div>

    <!-- Barre de filtres (Image 1) -->
    <form method="GET" class="filter-bar-premium">
        <div class="f-group">
            <label><i class="fas fa-search"></i> RECHERCHE</label>
            <input name="q" value="<?= h($filters['q']) ?>" placeholder="Titre de la série..."/>
        </div>
        ...
        <div class="f-group group-year">
            ...
            <input type="number" name="year_from" value="<?= $filters['year_from'] ?>" placeholder="1980"/>
        </div>

        <div class="f-group group-year">
            <label><i class="fas fa-calendar-check"></i> À</label>
            <input type="number" name="year_to" value="<?= $filters['year_to'] ?>" placeholder="2025"/>
        </div>

        <div class="f-group">
            <label><i class="fas fa-star"></i> NOTE MIN</label>
            <input type="number" step="0.1" name="min_rating" value="<?= $filters['min_rating'] ?>" placeholder="7.0"/>
        </div>
    ...
    <div class="grid">
        <?php if(empty($results)): ?>
            <div class="empty-s" style="grid-column: 1/-1;">
                ...
                <a href="series.php" class="btn-ghost">Réinitialiser</a>
            </div>
        <?php else: ?>
...
.f-group { flex: 1; display: flex; flex-direction: column; gap: 8px; }
.f-group label {
    font-size: 0.65rem;
    font-weight: 700;
...
.btn-filter-premium:hover { transform: translateY(-3px); box-shadow: 0 15px 30px rgba(230, 57, 70, 0.4); }
</style>
